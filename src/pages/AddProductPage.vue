...
export default {
  ...
  setup() {
    ...
    const onSubmit = async () => {
      ...
      try {
        // API'ye ürün ekle
        const response = await productAPI.create(product.value)
        
        // Son eklenen ürünü göster
        lastAddedProduct.value = response.data.data
    ...
    const handleImageUpload = async (file) => {
      if (file) {
        ...
        reader.onload = async (e) => {
          ...
          // Cloudinary'ye yükle
          try {
            $q.loading.show({ message: 'Resim yükleniyor...' })
            const response = await uploadAPI.uploadImage(base64)
            product.value.image = response.data.data.url
